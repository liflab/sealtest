 ...
 * in the given automaton, favoring edges that are not visited.
...
public class GreedyAutomatonGenerator extends GreedyTraceGenerator<AtomicEvent>
{
	 ...
	 */
	protected Set<Edge<AtomicEvent>> m_visited;
	...
	public GreedyAutomatonGenerator(Automaton aut, Random random, CoverageMetric<AtomicEvent,Float> metric) 
	{
		super(random, metric);
		m_automaton = aut;
		m_visited = new HashSet<Edge<AtomicEvent>>();
	}
	...
	public Trace<AtomicEvent> generateTrace(int length) 
	{
		...
		{
			Set<Edge<AtomicEvent>> outgoing_edges = new HashSet<Edge<AtomicEvent>>();
			outgoing_edges.addAll(current_vertex.getEdges());
			outgoing_edges.removeAll(m_visited);
			...
			String edge_label = chosen_edge.getLabel().getLabel();
			if (edge_label.compareTo("*") == 0)
			{
				List<AtomicEvent> alph = new ArrayList<AtomicEvent>();
				alph.addAll(m_automaton.getAlphabet());
				for (Edge<AtomicEvent> edge : ordered_edges)
				{
					if (edge != chosen_edge)
					{
						alph.remove(edge.getLabel());
					}
				}
				trace.add(alph.get(0));
			}
			else
			{
