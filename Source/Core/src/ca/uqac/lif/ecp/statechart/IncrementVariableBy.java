...
/**
 * Action that increments a state variable by a fixed value. The corresponding
 * state variable in the statechart must be of integer type.
 * @author Sylvain Hallé
 * @param <T> The type of events in the statechart 
 */
public class IncrementVariableBy<T extends Event> extends VariableAssignment<T,Integer> 
{
	/**
	...
	/**
	 * The maximum value the variable is allowed to take. Attempting to
	 * increment the variable past this value will cause the action to fail,
	 * sending the statechart into the trash state.
	 */
	protected final int m_maxValue;

	public IncrementVariableBy(String var_name, int increment)
	{
		this(var_name, increment, 100000);
	}

	public IncrementVariableBy(String var_name, int increment, int max_value)
	{
		super(var_name);
